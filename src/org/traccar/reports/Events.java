...
public final class Events {
    ...
    public static void getExcel(OutputStream outputStream,
        ...
        HashMap<Long, String> positionNames = new HashMap<>();
        for (long deviceId: ReportUtils.getDeviceList(deviceIds, groupIds)) {
            Context.getPermissionsManager().checkDevice(userId, deviceId);
            Collection<Event> events = Context.getDataManager().getEvents(deviceId, from, to);
            boolean all = types.isEmpty() || types.contains(Event.ALL_EVENTS);
            for (Iterator<Event> iterator = events.iterator(); iterator.hasNext();) {
                ...
                if (all || types.contains(event.getType())) {
                    ...
                    if (geofenceId != 0) {
                        ...
                        if (Context.getPositionManager().checkItemPermission(userId, positionId)) {
                            Position maintenance = Context.getPositionManager().getById(positionId);
                            if (maintenance != null) {
                                positionNames.put(positionId, maintenance.getAddress());
                            }
                        } else {
                            iterator.remove();
                        }
        ...
        try (InputStream inputStream = new FileInputStream(templatePath + "/events.xlsx")) {
            ...
            jxlsContext.putVar("maintenanceNames", maintenanceNames);
                        jxlsContext.putVar("addressNames", positionNames);
