...
Ext.define('Traccar.AttributeFormatter', {
    ...
    groupIdFormatter: function (value) {
	 ...
	 positionIdFormatter: function (value) {
        var position, store;
        if (value !== 0) {
            store = Ext.getStore('AllPositions');
            if (store.getTotalCount() === 0) {
                store = Ext.getStore('Positionss');
            }
            position = store.getById(value);
            return position ? position.get('address') : value;
        }
        return null;
    },
        ...
        switch (key) {
            ...
            case 'geofenceId':
                return this.geofenceIdFormatter;
				  case 'positionId':
                return this.positionIdFormatter;
            case 'maintenanceId':
